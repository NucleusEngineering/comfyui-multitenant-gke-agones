...
resource "null_resource" "build_comfyui_image" {
  provisioner "local-exec" {
    command     = "gcloud builds submit --machine-type=e2-highcpu-32 --disk-size=100 --region=us-central1 -t ${var.artifact_registry}/${var.sd_comfyui_image.tag}"
    working_dir = var.comfyui_comfyui_image.path
  }
}
...
resource "null_resource" "modify_nginx_image" {
  provisioner "local-exec" {
    command     = "sed -i 's/$${REDIS_HOST}/redis.private.domain/g' comfyui.lua"
