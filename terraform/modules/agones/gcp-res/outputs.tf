...
output "cloud_build_image_cmd_sample" {
  value       = "gcloud builds submit --machine-type=e2-highcpu-32 --disk-size=100 --region=${var.region} -t ${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.comfyui_repo.name}/comfyuiTAG"
...
output "artifactregistry_url" {
  value       = "${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.comfyui_repo.name}"
  description = "artifactregistry_url"
}
output "artifactregistry_name" {
  value       = "${google_artifact_registry_repository.comfyui_repo.name}"
  description = "artifactregistry_name"
}
output "comfyui_static_ip_addr_name" {
  value = "${google_compute_global_address.comfyui_addr.name}"
  description = "comfyui_static_ip_addr_name"
}