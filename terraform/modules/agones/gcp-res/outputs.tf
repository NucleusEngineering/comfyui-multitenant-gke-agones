...
output "cloud_build_image_cmd_sample" {
  value       = "gcloud builds submit --machine-type=e2-highcpu-32 --disk-size=100 --region=${var.region} -t ${var.region}-docker.pkg.dev/${var.project_id}/${google_artifact_registry_repository.sd_repo.name}/sd:TAG"
...
output "comfyui_address_name" {
  value       = google_compute_global_address.comfyui_addr.name
  description = "comfyui global static ip name"
}
output "comfyui_address" {
  value       = google_compute_global_address.comfyui_addr.address
  description = "comfyui global static ip address"
}
