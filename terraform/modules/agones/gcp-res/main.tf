...
resource "google_compute_global_address" "comfyui_addr" {
  name       = "comfyui-ingress-${random_id.tf_subfix.hex}"
  depends_on = [google_project_service.gcp_services]
}
...
resource "google_container_cluster" "gke" {
  ...
  node_pool_auto_config {
    network_tags {
      tags = var.gke_autopilot_network_tags
    }
  }
...
resource "google_artifact_registry_repository" "comfyui_repo" {
  location      = var.region
  repository_id = "comfyui-repository-${random_id.tf_subfix.hex}"
  description   = "ComfyUI repository"
  format        = "DOCKER"
  depends_on    = [google_project_service.gcp_services]
}
# Redis cache
resource "google_redis_instance" "cache" {
  region             = var.region
  name               = "comfyui-agones-cache-${random_id.tf_subfix.hex}"
  tier               = "BASIC"
  memory_size_gb     = 1
  authorized_network = google_compute_network.vpc.id
  redis_version      = "REDIS_7_2"
  display_name       = "ComfyUI Agones Cache Instance"
...
resource "google_cloud_scheduler_job" "job" {
  name        = "comfyui-agones-cron-${random_id.tf_subfix.hex}"
